...
static void spi_write(enum MODE mode, char message)
{
...
void nokia_5110_init(void)
{
	...
	spi_write(COMMAND, 0x21);              // chip active; vertical addressing mode (V = 1); use extended instruction set (H = 1)
										  // set LCD Vop (contrast), which may require some tweaking:
	spi_write(COMMAND, CONTRAST);          // try 0xB1 (for 3.3V red SparkFun), 0xB8 (for 3.3V blue SparkFun), 0xBF if your display is too dark, or 0x80 to 0xFF if experimenting
	spi_write(COMMAND, 0x04);              // set temp coefficient
	spi_write(COMMAND, 0x14);              // LCD bias mode 1:48: try 0x13 or 0x14

	spi_write(COMMAND, 0x20);              // we must send 0x20 before modifying the display control mode
	spi_write(COMMAND, 0x0C);              // set display control to normal mode: 0x0D for inverse
	delay(100);
}
...
void nokia_5110_char(unsigned char data){
	int i;
	for(i = 0; i < 5; ++i)
		spi_write(DATA, ASCII[data - 0x20][i]);

	spi_write(DATA, 0x00);                 // blank vertical line padding
}

void nokia_5110_string(char *ptr){
	while(*ptr) {
		nokia_5110_char((unsigned char)*ptr);
		++ptr;
	}
}
...
void nokia_5110_set_cursor(unsigned char column, unsigned char raw){
	...
	spi_write(COMMAND, 0x80|(column*7));     // setting bit 7 updates X-position
	spi_write(COMMAND, 0x40|raw);         // setting bit 6 updates Y-position
}
...
void nokia_5110_image(const char *data)
{
	int i;
	nokia_5110_set_cursor(0, 0);
	for(i = 0; i < (MAX_X * MAX_Y / 8); ++i)
		spi_write(DATA, data[i]);
}
